// use readline package to get user input
...
// start
rl.question(
    "Please enter a short increasing sequence separated by space:  ",
    function start(str) {
        solution(str);
        rl.question("Try another sequence or enter Q to exit):  ", function(
            input
        ) {
            if (input === "Q" || input === "q") {
                rl.close();
...
function solution(str) {
    ...
    const algos = [new arithmetic(), new geometric(), new fibonacci(), new MultiplyAndAdd(), new squareNumber(), new triangular()];
    ...
    algos.forEach(a => {
        ...
        if (possibleMatch) {
            console.log(
                `[Possibile Pattern] ${a.pattern}, [next 10 values] ${a.nextTen(seq)}`
            );
        }
    });
};
...
function geometric(seq) {
    ...
    this.pattern = "Geometric sequence";
    this.seq = seq;
    this.match = seq => {
...
function fibonacci(seq) {
    ...
    this.nextTen = seq => {
        let arr = [seq[seq.length - 1] + seq[seq.length - 2], 2 * seq[seq.length - 1] + seq[seq.length - 2]];
        for (let i = 0; i < 8; i++) {
...
function MultiplyAndAdd(seq) {
    this.pattern = "Multiply an integer and add any one";
    this.seq = seq;
    const multiplier = seq => (seq.length > 2 ? (seq[2] - seq[1]) / (seq[1] - seq[0]) : 0);
    this.match = seq => {
        if (seq.length < 3 || // too short to identify 
        ...
        return seq.every((v, i) => i > 1 ? (v - seq[i - 1]) / (seq[i - 1] - seq[i - 2]) === multiplier(seq) : true);
    };
    this.nextTen = seq => {
        let arr = [seq[seq.length - 1] + (seq[seq.length - 1] - seq[seq.length - 2]) * multiplier(seq)];
        arr.push(arr[0] + (seq[seq.length - 1] - seq[seq.length - 2]) * Math.pow(multiplier(seq), 2))
        for (let i = 2; i < 10; i++) {
...
function squareNumber(seq) {
    ...
    this.seq = seq;
    this.match = seq => {
        return seq.every((v, i) => Number.isInteger(Math.pow(v, 0.5))) &&
            seq.every((v, i) => (i > 1 ? (v - 2 * seq[i - 1] + seq[i - 2]) === 2 : true));
    }
    this.nextTen = seq => {
...
function triangular(seq) {
    ...
    this.match = seq => {
        ...
        return seq.every((v, i) => (i > 1 ? v - 2 * seq[i - 1] + seq[i - 2] === 1 : true));
    };
    this.nextTen = seq => {
